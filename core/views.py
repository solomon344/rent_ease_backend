...
from core.serializer import (ProfileSerializer,UserSerializer,AmentiesSerializer, BookingSerializer, PropertySerializer,PayoutSerializer)
from core.models import (Profile,User,Amenties,Property,Booking,PropertyMedia,Payout)
...
class PropertyFilterSet(FilterSet):
    ...
    # Price range filters
    price__gte = NumberFilter(field_name='price', lookup_expr='gte',distinct=True)
    price__lte = NumberFilter(field_name='price', lookup_expr='lte',distinct=True)

    # Amenities filter - supports filtering by multiple amenities IDs
    amenities__id__in = BaseInFilter(field_name='amenities__id', lookup_expr='in', distinct=True)
...
class PropertyView(ListAPIView):
    ...
    search_fields = ['name','description','distinct']
...
class BookingAcceptView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if status == 'confirmed':
            ...
            # Add refresh flag to response
            return Response({
                "message": f"Booking from {booking.user.first_name} {booking.user.last_name} has been accepted and payment link sent",
...
class PayoutViewSet(viewsets.ModelViewSet):
    ...
    def last(self, request):
        last_payout = Payout.objects.filter(user=request.user).order_by('-created_at').first()
        if last_payout:
            return Response({
                'amount': last_payout.amount,
                'date': last_payout.created_at,
    ...
    def available_amount(self, request):
        ...
        # Calculate remaining payoutable amount
        remaining_amount = total_revenue - total_payouts
    ...
    def create(self, request):
        ...
        if not all([amount, mobile_money_number, payment_method]):
            return Response({"message": "Missing required fields"}, status=400)

        # Check if user has made a payout in the last 24 hours
        ...
        if last_payout:
            return Response({"message": "You can only request a payout once every 24 hours"}, status=400)

        try:
            # Initialize ModemPay client
            ...
            if float(amount) > remaining_amount:
                return Response({
                    "message": f"You can only request a payout up to {remaining_amount}",
                    "remaining_amount": remaining_amount
                }, status=400)

            # Create payout payload
            payload = {
                'amount': float(amount),  # Convert to cents
                'currency': 'GMD',
                'network': payment_method,
                'account_number':mobile_money_number,
                'beneficiary_name': request.user.first_name + " " + request.user.last_name,
                'narration':"RentEase Payout",
                'description': f"RentEase payout for {request.user.email}",
                'metadata': {
            ...
            # Create payout
            payout_response = modempay_client.transfers.initiate(payload, idempotency_key=str(uuid.uuid4()))

            if payout_response and payout_response.get('status') == 'success':
                ...
                payout = Payout.objects.create(
                    ...
                    payment_method=payment_method,
                    status='pending',
                    transaction_id=payout_response.get('data', {}).get('id')
                )
                ...
                return Response({
                    ...
                    "success": False,
                    "error": payout_response.get('message', 'Unknown error')
                }, status=500)

        except Exception as e:
            logger.error(f"Payout creation failed: {str(e)}", exc_info=True)
            return Response({
...
class PaymentCallbackView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if transaction and transaction.get('status') == 'completed':
            ...
            # Send lease agreement email to owner
            safe_send_html_email(
                subject="Digital Lease Agreement - Rental Booking",
                recipient_list=[owner.email],
                template_name="lease-agreement-email.html",
                context={
                    'property_address': booking.property.location,
                    'renter_name': f"{booking.user.first_name} {booking.user.last_name}".strip(),
                    'lease_start_date': booking.start_date,
                    'lease_end_date': booking.end_date,
                    'monthly_rent': booking.total_price,
                    'security_deposit': booking.property.price * 0.5,  # Assuming 50% of price as security deposit
                    'payment_method': 'Mobile Money',
                },
            )

            # Send lease agreement email to renter
            safe_send_html_email(
                subject="Digital Lease Agreement - Rental Booking",
                recipient_list=[booking.user.email],
                template_name="lease-agreement-renter.html",
                context={
                    'renter_name': f"{booking.user.first_name} {booking.user.last_name}".strip(),
                    'property_address': booking.property.location,
                    'landlord_name': f"{owner.first_name} {owner.last_name}".strip(),
                    'lease_start_date': booking.start_date,
                    'lease_end_date': booking.end_date,
                    'monthly_rent': booking.total_price,
                    'security_deposit': booking.property.price * 0.5,  # Assuming 50% of price as security deposit
